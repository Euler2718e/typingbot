# TypingBot

A tiny macOS menu bar app that types prepared text with human-ish rhythm.

TypingBot lets you paste a block of text, arm it, focus any text field, and have the app type it for you. It is intentionally a little silly, but the implementation is native macOS: Swift, AppKit, SwiftUI, CoreGraphics event taps, and synthetic keyboard events.

The goal is not to be a stealth tool or a shortcut for spam. It is a compact side project about keyboard automation, macOS permissions, and making generated typing feel less mechanically perfect.

## Features

- Global hotkeys:
  - `Cmd+Opt+O` opens the control panel.
  - `Cmd+Opt+T` starts, pauses, and resumes typing.
  - `Esc` pauses typing.
  - `Esc` twice hard-resets the run so the next start begins from the top.
- Human-like typing simulation:
  - Configurable typing speed.
  - Easy words type faster.
  - Harder words type slower and can be more error-prone.
  - Higher speeds produce fewer typos.
  - Optional corrected typos and transpositions.
- Native macOS permissions flow for Accessibility and Input Monitoring.
- Minimal menu bar UI with local settings persistence.

## Screenshot

![TypingBot screenshot](screenshot.png)

## Install

Download `TypingBot-macOS.zip` from the [Releases](https://github.com/Euler2718e/typingbot/releases) page.

1. Download `TypingBot-macOS.zip`.
2. Unzip it.
3. Open `TypingBot.app`.
4. If macOS blocks the app on first launch, right-click `TypingBot.app` and choose `Open`.
5. Grant Accessibility and Input Monitoring when prompted.

TypingBot needs those permissions because macOS protects global keyboard listeners and apps that type into other apps. Accessibility allows TypingBot to send keystrokes. Input Monitoring allows TypingBot to listen for global hotkeys like `Cmd+Opt+T`.

## Usage

1. Press `Cmd+Opt+O` to open the panel.
2. Paste or type the text you want TypingBot to write.
3. Adjust speed and typo settings.
4. Click `Arm`.
5. Focus the app and text field where the text should go.
6. Press `Cmd+Opt+T` to start.

While TypingBot is running, it suppresses physical key presses so accidental keyboard input does not interfere with the generated text. Press `Esc` once to pause. Press `Esc` again while paused to reset the run to the beginning.

## Build From Source

Requirements:

- macOS 13 or newer
- Xcode Command Line Tools
- Swift Package Manager

Clone and build:

```bash
git clone https://github.com/Euler2718e/typingbot.git
cd typingbot
./build_mac.sh
open dist/TypingBot.app
```

The build script compiles the Swift package, assembles a `.app` bundle in `dist/`, and applies an ad-hoc code signature so macOS can track the app consistently in Privacy & Security settings.

## Architecture

TypingBot is a small native macOS app:

- AppKit owns the menu bar lifecycle and permission setup window.
- SwiftUI renders the popover control panel.
- CoreGraphics event taps handle global hotkeys and keyboard suppression.
- Synthetic `CGEvent`s type the prepared text into the focused app.
- `HumanSim` controls timing, speed variation, corrected mistakes, transpositions, and word difficulty behavior.

Most of the interesting work is in making the app feel simple from the outside while respecting macOS security boundaries under the hood.

## Privacy

- No analytics.
- No network calls.
- No server.
- Your prepared text stays on your Mac.
- Accessibility and Input Monitoring are used only for global hotkeys, keyboard suppression, and typing into the focused app.

## Publishing A Release

These are the exact steps I use to publish a downloadable build.

### 1. Create The GitHub Repository

Go to GitHub and create a new repository:

- Repository name: `typingbot`
- Visibility: Public
- Do not add a README, license, or `.gitignore` on GitHub.

The local project already includes those files.

### 2. Push The Local Repository

From the project folder:

```bash
git branch -M main
git remote add origin https://github.com/Euler2718e/typingbot.git
git push -u origin main
```

If GitHub asks you to sign in, follow the browser prompt or use a personal access token.

### 3. Build The App Zip

```bash
./build_mac.sh
cd dist
zip -r TypingBot-macOS.zip TypingBot.app
```

This creates `dist/TypingBot-macOS.zip`.

### 4. Draft A GitHub Release

On GitHub:

1. Open the `typingbot` repository.
2. Go to `Releases`.
3. Click `Draft a new release`.
4. Tag: `v1.0.0`
5. Title: `TypingBot v1.0.0`
6. Upload `dist/TypingBot-macOS.zip`.
7. Use these release notes:

```md
Initial public release of TypingBot.

Includes:
- Native macOS menu bar app
- Global hotkeys
- Human-like typing simulation
- Configurable speed and typo behavior
- Local-only operation

Note: macOS requires Accessibility and Input Monitoring permissions for TypingBot to work.
```

## Verification Before Publishing

Run:

```bash
swift build -c release
./build_mac.sh
codesign --verify --deep --strict --verbose=2 dist/TypingBot.app
git status
```

`git status` should only show intentional source and documentation changes. Build outputs such as `.build/` and `dist/` should stay untracked.

## License

MIT
