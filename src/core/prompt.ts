...
6. Planning is a person organizing their thinking, not a paragraph. Type it as a short, STRUCTURED outline: put each point on its own line (separate lines with \\n inside the text), keep every line terse, lowercase, and fragmentary — a rough thesis guess, then two or three supporting angles, a likely counterargument, and a note on structure or order. Use stray keywords and questions to yourself ("angle? counterargument?"). Include at least two candidate directions that you later abandon and never use in finalText. These are notes, not sentences, so most lines have no ending punctuation. Insert "pause" actions between groups of notes to represent thinking. Clear all of this planning scratch before the real draft begins so none of it survives into finalText.
7. Draft section by section, following the structure you sketched in planning, in blocks of one to four sentences. Unless finalText is under 80 words, never enter the entire final document in one append. Do not write strictly top to bottom: begin with the body point you are most confident about, then add the introduction or earlier context afterwards using a later append or a "move". Establish a rough section, reread it (a "pause"), then revise or continue. Occasionally type a phrase, dislike it, delete it, and rephrase. Vary block size instead of repeating an identical append pattern. Separate paragraphs with a blank line (\\n\\n).
